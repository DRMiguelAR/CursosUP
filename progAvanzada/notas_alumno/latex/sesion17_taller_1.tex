...
Esta sesión es trabajo \textbf{individual}.  Tienes dos problemas; el objetivo
es obtener \texttt{AC} en ambos antes de que termine la clase.

\begin{consejo}
  \textbf{Flujo recomendado para cada problema:}
  \begin{enumerate}
    \item Lee el enunciado completo. No empieces a codificar todavía.
    \item Resuelve los ejemplos a mano y verifica que entiendes la salida
          esperada.
    \item Piensa en la \textit{condición matemática} que determina la respuesta.
    \item Escribe el algoritmo en pseudocódigo (una oración por paso).
    \item Implementa en C++, prueba con los ejemplos del enunciado.
    \item Envía al juez.  Si recibes \texttt{WA}, busca un contraejemplo
          antes de cambiar código al azar.
  \end{enumerate}
\end{consejo}
