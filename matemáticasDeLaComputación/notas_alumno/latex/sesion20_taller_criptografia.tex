...
César} (\textit{shift cipher}), el \textbf{cifrado afín}
...
\begin{recuerda}
  ...
  Los tres sistemas operan exclusivamente sobre los 43~caracteres de
...
(de \texttt{00} a \texttt{42}).  El tamaño del alfabeto es $N = 43$.
...
\begin{advertencia}
  Los caracteres con tilde (Á, É, Í, Ó, Ú) \textbf{no están} en la
  tabla.  Al convertir tu frase a mayúsculas, elimina las tildes:
  Á\,$\to$\,A, É\,$\to$\,E, Í\,$\to$\,I, Ó\,$\to$\,O, Ú\,$\to$\,U.
  La \textbf{Ñ sí está} incluida (código 26); no la reemplaces.
\end{advertencia}
...
El \textbf{cifrado César} desplaza el código de cada carácter un número
fijo $k$ de posiciones módulo $N = 43$.  Para codificar el carácter con
código $m_i$:
...
\begin{consejo}
  ...
  $m_i = (c_i - k + 43) \bmod 43$ y comprueba que recuperas el texto
  original.
\end{consejo}
...
\[
  c_i = (a \cdot m_i + b) \bmod 43
\]

Para que la función sea invertible, $a$ debe ser coprimo con $N = 43$.

\begin{recuerda}
  Como $43$ es primo, \textbf{cualquier} $a \in \{2, 3, \ldots, 42\}$
  es coprimo con 43.  El valor $a = 1$ equivale a un cifrado César.
\end{recuerda}

Al igual que en el cifrado César, cada $c_i$ es el código de un
carácter de la tabla; entrega la frase cifrada como cadena de
caracteres.
...
\begin{advertencia}
  Verifica que $M_i < n$ para todo bloque.  Como $M_i \le 424\,242$
