...
\begin{recuerda}
  ...
  Los tres sistemas operan exclusivamente sobre los 50~caracteres de
...
(de \texttt{00} a \texttt{49}).  El tamaño del alfabeto es $N = 50$.
...
Al responder el Forms deben escribir únicamente el \textbf{nombre del equipo}.
...
\begin{itemize}
  \item Solo el verso o estrofa — \textbf{no incluyas} el título ni
        el nombre del artista en la frase a cifrar; esos datos se
        reportan por separado en el Forms.
  \item Escrita originalmente en español (no traducción).
  \item Claramente identificable por cualquier persona.
  \item Al menos una palabra de la frase debe ser un
        \textbf{sustantivo singular común} en español.
  \item Sin groserías ni doble sentido ofensivo.
  \item Cada cifrado debe usar una \textbf{canción y frase distintas}.
  \item Una vez convertida a mayúsculas, debe contener únicamente
        caracteres de la tabla de la sección~\ref{sec:tabla}.
\end{itemize}

\begin{consejo}
  La frase debe tener \textbf{más de 20 caracteres} (ya en mayúsculas).
  No hay límite superior, pero entre más larga, más bloques RSA tendrás
  que calcular.
\end{consejo}

\begin{advertencia}[¡Tus frases son secretas!]
  Las frases que registres son el material de la \textbf{siguiente
  actividad}: otros equipos intentarán decodificarlas e identificar
  a qué canción pertenecen.  Será un \textbf{concurso} — el equipo
  que logre decodificar más frases gana.

  \medskip
  \textbf{No compartas tu verso ni tus claves} con otros equipos.
  Hacerlo echa a perder la siguiente actividad para todos.
\end{advertencia}
...
\begin{enumerate}
  ...
  \item \textbf{Clave de decodificación} — para RSA y cifrado afín.
\end{enumerate}
...
de cada carácter, módulo $N = 50$.  Para codificar el carácter con
código $m_i$:
...
\begin{consejo}
  ...
  $m_i = (c_i - k) \bmod 50$ y comprueba que recuperas el texto
  original.
\end{consejo}
...
\[
  c_i = (a \cdot m_i + b) \bmod 50
\]

Para que la función sea invertible, $a$ debe ser coprimo con $N = 50$.

\begin{recuerda}
  Como $50 = 2 \times 5^2$, un valor $a$ es coprimo con 50 si y solo si
  $a$ \textbf{no es divisible} ni por 2 ni por 5.  Por ejemplo: 3, 7,
  9, 11, 13, 17, 19, 21\ldots\ son válidos; 4, 5, 10, 15 no lo son.
...
\begin{description}
  \item[Clave de codificación a reportar.] El par $(a,\, b)$ que
        elegiste.
  \item[Clave de decodificación a reportar.] El inverso modular
        $a^{-1} \bmod 50$, es decir, el entero $a'$ tal que
        $a \cdot a' \equiv 1 \pmod{50}$.
\end{description}
...
\begin{advertencia}
  Verifica que $M_i < n$ para todo bloque.  Como $M_i \le 494\,949$
...
Afín    & $(a,\, b)$ & $a^{-1} \bmod 50$ & cadena de caracteres \\
...
\begin{recuerda}[Calificación]
  Esta actividad se califica con dos criterios:
  \begin{enumerate}
    \item \textbf{Entrega completa.}  Subiste toda la información
          solicitada al Forms para los tres cifrados.
    \item \textbf{Frases decodificables.}  Cada frase cifrada puede
          recuperarse exactamente aplicando el algoritmo correspondiente
          con los parámetros que reportaste.
  \end{enumerate}
  No importa qué verso elijas ni qué claves uses, siempre que la
  decodificación funcione.
\end{recuerda}
